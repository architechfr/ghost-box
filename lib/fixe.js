...
window.GBFixe = (function(){
  ...
  css.textContent = [
    ...
    '#gbxBar{display:none}',
    /* le bandeau est de l'INFORMATION, pas de l'écran : chaque pixel qu'il
       prend est volé au mur et à l'image. Resserré après retour d'usage —
       « toute cette place perdue en haut, les mots sont tassés en bas ». */
    'body.gbx #gbxBar{display:flex;flex-wrap:wrap;align-items:center;gap:4px;',
      'position:fixed;left:0;right:0;top:0;z-index:60;',
      'padding:calc(4px + env(safe-area-inset-top, 0px)) 50px 5px 8px;',
    ...
    '.gbx-chip{display:inline-flex;align-items:baseline;gap:4px;padding:2px 6px;border-radius:2px;',
    ...
    '.gbx-chip .v{font-size:11px;color:#DDE6EA}',
    ...
    /* retour d'usage : « je n'avais pas compris que ces rectangles étaient des
       paramètres que je pouvais modifier ». Les pastilles d'état (coins presque
       carrés, fond teinté, pas de relief) et les BOUTONS (coins arrondis, fond
       sombre franc, bordure plus nette) doivent se distinguer au premier regard. */
    '.gbx-mods button{font-family:"IBM Plex Mono",ui-monospace,monospace;font-size:9px;',
      'letter-spacing:.08em;text-transform:uppercase;padding:4px 8px;border-radius:7px;',
      'border:1px solid #3A4750;background:rgba(13,20,24,.92);color:#8A99A2;cursor:pointer}',
    '.gbx-mods button.on{border-color:#5FD3A6;color:#5FD3A6}',
    /* le bouton d'action (objectif) : MÊME typographie que les autres —
       demande explicite, une fois compris que tout le bandeau se touche.
       Seule la bordure légèrement verte le distingue encore, sans un pixel
       de plus. Le ⟳ a vécu : il coûtait de la place pour rien. */
    '.gbx-mods button.acte{border-color:#3E6B5A;color:#CFC6B4}',
    '.gbx-mods button:active{transform:translateY(1px)}',
    /* ── la sortie : une CROIX, pas une phrase ──
       « Quitter l'écran fixe » écrit en toutes lettres mangeait 92 px de
       bandeau et donnait l'impression qu'un texte se cachait derrière. Une
       croix en haut à droite est comprise partout ; elle ne réserve plus que
       50 px, rendus aux pastilles d'information. */
    '#gbxQuit{display:none}',
    'body.gbx #gbxQuit{display:flex;align-items:center;justify-content:center;',
      'position:fixed;z-index:62;right:8px;top:calc(env(safe-area-inset-top, 0px) + 5px);',
      'width:36px;height:36px;padding:0;border-radius:50%;cursor:pointer;',
      'font-family:ui-monospace,monospace;font-size:17px;line-height:1;font-weight:400;',
      'letter-spacing:0;text-transform:none;',
      'background:rgba(4,8,11,.92);border:1px solid rgba(95,211,166,.45);color:#5FD3A6}',
    'body.gbx #gbxQuit:active{transform:scale(.94)}'
  ].join('');
  ...
  function installer(c){
    ...
    if(!quit){
      ...
      quit.textContent = '✕'; quit.setAttribute('aria-label', "Quitter l'écran fixe");
      quit.title = "Quitter l'écran fixe";
