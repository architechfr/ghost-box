/* ═══════════════════════════════════════════════════════════════════════
   ...
   ── LA PLANCHETTE : UNE RONDE, PAS UNE DÉRIVE ──
   Elle passe devant CHAQUE glyphe, dans un ordre tiré au sort, à un rythme
   régulier, et recommence une fois le tour fini. Rien dans ce mouvement ne
   veut dire quoi que ce soit : c'est un métronome qui parcourt la planche.

   La version précédente faisait dériver la planchette au gré des valeurs
   vivantes des capteurs. C'était plus joli et c'était PIRE : un mouvement
   corrélé aux capteurs se lit comme une intention — on regarde la planchette
   ralentir près d'un E et on y voit quelque chose, alors que rien n'a été
   retenu. Une ronde mécanique, elle, ne se laisse pas interpréter. C'est le
   même raisonnement que le témoin de bruit : rendre le hasard visible pour
   qu'il cesse d'être pris pour un signe.

   Une lettre RETENUE interrompt la ronde : la planchette s'y rend, s'y
   arrête le temps demandé, et le glyphe est MARQUÉ — couleur différente,
   marque qui persiste plusieurs secondes après. C'est le seul moment où
   quelque chose est dit, et il ne doit ressembler à aucun autre.
...
window.GBPlanche = (function(){
  ...
  /* ── Géométrie de la planche, en pourcentages du cadre (0–100) ──
     Deux dômes de treize lettres, les chiffres en ligne, les trois réponses.
     Chaque glyphe porte aussi sa ROTATION : sur une vraie planche les lettres
     sont gravées le long de l'arc, elles ne restent pas droites. La rotation
     est la tangente de l'ellipse, corrigée du RAPPORT DU CADRE — sans cette
     correction les lettres du bas partent de travers, parce qu'un pourcentage
     de largeur et un pourcentage de hauteur ne valent pas le même nombre de
     pixels. C'est le même piège que `min-width:auto` : un calcul juste dans
     un espace qui n'est pas celui de l'écran. */
  var RATIO = 1.55;            // largeur / hauteur du cadre, cf. planche/index.html

  /* Un ARC DE CERCLE, pas une ellipse. Première version : une ellipse très
     plate (Rx=45 en % de largeur, Ry=25 en % de hauteur) — sa tangente aux
     extrémités est VERTICALE, donc le A et le M se retrouvaient couchés à 90°.
     Sur une planche gravée, les lettres pivotent doucement, d'une quarantaine
     de degrés au bout : c'est la signature d'un arc de cercle vu en vrai, pas
     d'une ellipse écrasée. Le cercle est défini en PIXELS puis converti : un
     rayon exprimé en % de hauteur devient, horizontalement, R/RATIO en % de
     largeur. La rotation d'un glyphe est alors simplement son angle θ.
     Les deux contraintes (demi-largeur et flèche) fixent R et l'angle :
       demi-largeur = (R/RATIO)·sin a     flèche = R·(1 − cos a)
     A→M : ±42 % de large, 25 % de flèche → a = 42,0°, R = 97,3
     N→Z : ±35 % de large, 17 % de flèche → a = 34,6°, R =  95,5
     (±45 % au premier essai : une fois PIVOTÉ de 42°, le A débordait du
      cadre — la boîte d'un glyphe tourné est plus large que le glyphe.)   */
  var ARC1 = { apex:32, R:97.3, a:42.0 };   // A → M
  var ARC2 = { apex:58, R:95.5, a:34.6 };   // N → Z

  function surArc(A, i, n){
    var deg = -A.a + (n > 1 ? (2*A.a*i)/(n-1) : A.a);
    var th = deg * Math.PI/180;
    return { x: 50 + (A.R/RATIO)*Math.sin(th),
             y: A.apex + A.R - A.R*Math.cos(th),
             rot: deg };
  }

  function positions(){
    var G = glyphes(), out = [], i, p;
    for(i = 0; i < 13; i++){ p = surArc(ARC1, i, 13); out.push({ ch:G[i], x:p.x, y:p.y, rot:p.rot, k:'l' }); }
    for(i = 0; i < 13; i++){ p = surArc(ARC2, i, 13); out.push({ ch:G[13+i], x:p.x, y:p.y, rot:p.rot, k:'l' }); }
    for(i = 0; i < 10; i++) out.push({ ch:G[26+i], x: 14 + i*8, y: 84, k:'d', rot:0 });
    out.push({ ch:'OUI', x:24, y:25, k:'r', rot:0 },
             { ch:'NON', x:76, y:25, k:'r', rot:0 },
             { ch:'AU REVOIR', x:50, y:95, k:'r', rot:0 });
    return out;
  }

  /* le tracé d'un dôme, pour le filet gravé sous les lettres.
     `ecart` en % de hauteur : positif = plus bas que la ligne des lettres. */
  function cheminArc(A, ecart){
    var d = '', i, p, n = 60;
    A = A || ARC1; ecart = ecart || 0;
    var B = { apex: A.apex + ecart, R: A.R, a: A.a };
    for(i = 0; i <= n; i++){
      p = surArc(B, i, n + 1);
  ...
  function arcs(){ return { un:ARC1, deux:ARC2, ratio:RATIO }; }
  ...
  function creer(opts){
    ...
    /* Repos en HAUT AU CENTRE, et dérive dans la bande libre au-dessus des
       lettres. Première version : dérive sur 24–76 % de hauteur, c'est-à-dire
       en plein sur les deux arcs — la planchette masquait des lettres en
       permanence, y compris pendant la garde où elle ne désigne rien. Elle ne
       descend sur la planche que lorsqu'elle a une lettre à montrer. */
    var plan = { x:50, y:21 };
    var cible = null, dwell = 0, pend = null, courant = '';
    var journal = [], refus = [];
    /* la ronde : l'ordre est tiré une fois, refait à chaque tour complet */
    var RONDE = opts.ronde != null ? opts.ronde : 0.85;   // s par glyphe
    var ordre = [], pos = 0, pause = 0, tours = 0;
    var marque = null, MARQUE_S = 6;                       // s de persistance
    /* la main : on peut pousser la planchette au doigt. Ça n'enregistre RIEN
       et ça ne peut RIEN désigner — c'est un geste, pas une mesure. Mais le
       geste compte : une planche qu'on ne peut pas toucher n'est pas une
       planche. Tant que la main tient, la ronde est suspendue et l'écran le
       dit, pour que personne ne prenne un mouvement de doigt pour une réponse. */
    var main = null;            // {x,y} tant que le doigt tient
    var tOrg = Math.random()*100;   // phase de la respiration, propre à la séance

    function rnd(){ g = (g*1664525 + 1013904223) % 4294967296; return g/4294967296; }

    /* mélange de Fisher-Yates avec le générateur du module : en mode témoin,
       la ronde elle-même est donc rejouable à l'identique depuis la graine. */
    function melanger(){
      ordre = []; for(var i = 0; i < P.length; i++) ordre.push(i);
      for(var j = ordre.length - 1; j > 0; j--){
        var k = Math.floor(rnd()*(j+1)), t = ordre[j]; ordre[j] = ordre[k]; ordre[k] = t;
      }
      pos = 0; tours++;
    }
    melanger();
    ...
    return {
      ...
      /* ── LA RONDE ──
         `lectures` n'entre plus dans le déplacement : la planchette ne suit
         plus les capteurs, elle fait sa ronde. Le paramètre reste pour que
         la page n'ait pas à changer d'appel, et parce que les valeurs servent
         encore aux jauges. Voir l'en-tête : un mouvement corrélé aux capteurs
         se lit comme une intention, une ronde mécanique non. */
      avancer: function(lectures, dt){
        // la marque du dernier glyphe retenu s'efface toute seule
        if(marque){ marque.age += dt; if(marque.age > MARQUE_S) marque = null; }

        /* ── LA MAIN PASSE AVANT TOUT ──
           Tant qu'un doigt tient la planchette, elle le suit, la ronde est
           gelée et aucune lettre ne peut être écrite : `cible` est relâchée.
           C'est la seule façon honnête d'autoriser le geste — sinon un
           déplacement à la main finirait par produire une lettre, et ce serait
           l'utilisateur qui écrirait le message sans le savoir. */
        if(main){
          plan.x += (main.x - plan.x) * Math.min(1, dt*14);
          plan.y += (main.y - plan.y) * Math.min(1, dt*14);
          cible = null; dwell = 0; pend = null;
          return null;
        }

        var enRoute = cible !== null;
        var idx = enRoute ? cible : ordre[pos];
        var t = P[idx];

        /* ── LA RESPIRATION ──
           « Le déplacement est trop fixe à mon goût. » Une interpolation pure
           vers un point donne un mouvement de machine-outil : trajectoire
           rectiligne, arrivée nette, arrêt parfait. Deux sinus lents et
           déphasés (périodes non commensurables, donc jamais de boucle
           visible) décalent la cible de moins de 1,5 % du cadre : la
           planchette respire, dérive un peu autour de la lettre, repart. Ça
           ne change RIEN à ce qu'elle désigne — l'écart est trop petit pour
           déplacer la loupe d'une lettre à l'autre — et tout à ce qu'elle a
           l'air d'être. */
        tOrg += dt;
        var souffle = enRoute ? 0.45 : 1;
        var ox = Math.sin(tOrg*0.61) * 1.45 * souffle + Math.sin(tOrg*0.23 + 1.7) * 0.75 * souffle;
        var oy = Math.cos(tOrg*0.47 + 0.9) * 1.05 * souffle + Math.sin(tOrg*0.19) * 0.55 * souffle;
        t = { x: t.x + ox, y: t.y + oy };

        /* la planchette pèse : elle glisse, elle ne saute pas. Elle va plus
           vite vers une lettre RETENUE que pendant la ronde — c'est le seul
           endroit où sa vitesse veut dire quelque chose, et ça se voit. */
        var k = Math.min(1, dt * (enRoute ? 3.0 : 2.2/Math.max(0.2, RONDE)));
        plan.x += (t.x - plan.x) * k;
        plan.y += (t.y - plan.y) * k;

        var dx = P[idx].x - plan.x, dy = P[idx].y - plan.y;
        var arrive = Math.sqrt(dx*dx + dy*dy) < 3.0;   // mesurée sur la VRAIE lettre,
                                                       // pas sur la cible qui respire

        if(!enRoute){
          dwell = 0;
          if(arrive){
            pause += dt;
            /* pause variable : entre 25 % et 95 % du rythme. Un métronome
               parfaitement régulier s'entend comme une machine ; l'irrégularité
               ne cache rien, elle empêche seulement l'œil de se caler dessus. */
            if(pause >= RONDE*(0.25 + rnd()*0.70)){
              pause = 0; pos++;
              if(pos >= ordre.length) melanger();   // tour fini : on retire l'ordre
            }
          }
          return null;
        }
        ...
        dwell += dt;
        if(dwell < arret) return null;

        var e = P[cible], sortie = {
          ch: e.ch, code: pend.code, label: pend.label, brut: pend.brut,
          unite: pend.unite, sigma: pend.sigma, idx: cible,
          source: pend.source, origine: pend.origine || 'capteurs', quand: Date.now()
        };
        journal.unshift(sortie);
        if(e.ch.length === 1) courant += e.ch;    // OUI / NON / AU REVOIR ne s'épellent pas
        marque = { idx: cible, ch: e.ch, origine: sortie.origine, age: 0 };
        cible = null; dwell = 0; pend = null;
        return { pose: sortie };
      },
      /* ancien nom, gardé le temps que toutes les pages suivent */
      deriver: function(lectures, dt){ return this.avancer(lectures, dt); },

      /* ── une lettre ENTENDUE ──
         Source distincte des capteurs : elle ne passe ni par le moteur, ni
         par la garde, ni par la coïncidence — c'est quelqu'un qui a parlé, il
         n'y a rien à mesurer. Elle est donc marquée « entendue » partout, et
         ne doit jamais être confondue avec une lettre désignée. */
      entendre: function(ch, info){
        if(cible !== null) return { ok:false, raison:'planchette déjà en route' };
        var i = -1;
        for(var k = 0; k < P.length; k++) if(P[k].ch === ch){ i = k; break; }
        if(i < 0) return { ok:false, raison:'glyphe inconnu : ' + ch };
        cible = i; dwell = 0;
        pend = { code:'voix', label:'micro (parole)', brut:(info && info.texte) || ch,
                 unite:'', sigma:0, source:'lettre prononcée', origine:'entendue' };
        return { ok:true, idx:i, ch:ch };
      },
      ronde: function(v){ if(v != null && isFinite(v)) RONDE = v; return RONDE; },
      /* ── la main ── x et y en % du cadre. `saisir` gèle la ronde, `lacher`
         la reprend là où elle en était : le tour n'est pas perdu. */
      saisir:  function(x, y){ main = { x:x, y:y }; },
      deplacer:function(x, y){ if(main){ main.x = x; main.y = y; } },
      lacher:  function(){ main = null; },
      enMain:  function(){ return !!main; },
      marque: function(){ return marque; },
      ...
      designer: function(feux){
        ...
        if(temoin){
          ...
          pend = { code:'temoin', label:'mode témoin', brut:'—', unite:'',
                   sigma:0, source:'graine '+graine, origine:'temoin' };
        ...
        pend = { code:dom.code, label:dom.label || dom.code,
                 ...
                 source:'partie fine de la valeur brute', origine:'capteurs' };
      ...
      etat: function(){
        return {
          ...
          survole: cible === null ? ordre[pos] : cible,
          main: !!main,
          origine: pend ? pend.origine : null,
          marque: marque,
          phase: main ? 'main' : cible === null ? 'ronde' : (dwell > 0.15 ? 'arret' : 'trajet')
        };
      },
  ...
  return { creer:creer, glyphes:glyphes, positions:positions, cheminArc:cheminArc,
           indexDe:indexDe, partieFine:partieFine, epreuve:epreuve, arcs:arcs,
