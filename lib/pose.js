...
export const VIS = 0.6;         // confiance minimale d'un point articulaire
export const MIN_POINTS = 14;   // points fiables exigés (sur 33) pour parler de « personne »
export const MIN_MEAN = 0.62;   // confiance moyenne exigée
export const MAX_PEOPLE = 4;    // personnes suivies simultanément
export const MIN_STREAK = 5;    // images consécutives avant d'afficher quoi que ce soit
export const MIN_HEIGHT = 0.14; // une personne occupe au moins 14 % de la hauteur de l'image
...
export function stats(){ return { streak: streak, rejected: rejected }; }
...
export function detect(video, ts){
  ...
  if(!res || !res.landmarks || !res.landmarks.length){ streak = 0; return null; }
  ...
  if(!people.length){ if(streak > 0) rejected++; streak = 0; return null; }
