...
function judge(lms){
  ...
  for(const i of TORSE){
    const q = lms[i];
    if(q && (q.visibility == null || q.visibility >= VIS)){
  ...
  const shapeOk = h >= MIN_HEIGHT && h > w * 0.55;   // assis = plus trapu que debout
  return { ok: strong >= MIN_POINTS && mean >= MIN_MEAN && shapeOk && structOk,
