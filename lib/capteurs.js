...
window.GBCapteurs = (function(){
  ...
  var VOIES = [
    ...
    { code:'cam_zone',   label:'Changement localisé',  unite:'0–255',famille:'camera',
      quoi:'Le petit coin de l\'image qui s\'écarte le plus de son fond appris — une petite lueur ou un voile léger y ressortent, là où la moyenne globale les noie. Ce qui touche TOUTE l\'image (exposition automatique, lampe) est soustrait : seul le local compte.' }
  ];
  ...
  function creer(opts){
    ...
    var zone=null;      // changement localisé — l'implémentation vit dans lib/zone.js
    var CW=64, CH=48;
    async function activerCamera(contraintes){
      if(vstream) return vstream;
      // l'objectif choisi la fois d'avant est repris ; à défaut c'est la caméra
      // ARRIÈRE — celle qu'on tourne vers la pièce, jamais vers soi
      if(!contraintes){
        var garde = null;
        try{ garde = localStorage.getItem('gb-objectif'); }catch(e){}
        contraintes = garde
          ? {video:{deviceId:{exact:garde}}, audio:false}
          : {video:{facingMode:{ideal:'environment'}}, audio:false};
      }
      try{
        vstream = await navigator.mediaDevices.getUserMedia(contraintes);
      }catch(e){
        // l'objectif mémorisé n'existe plus (autre téléphone, caméra occupée) :
        // on retombe sur l'arrière plutôt que d'échouer
        try{ vstream = await navigator.mediaDevices.getUserMedia(
          {video:{facingMode:{ideal:'environment'}}, audio:false}); }
        catch(e0){
      ...
      // le changement localisé exige son module ; s'il n'est pas chargé sur la
      // page, la voie le DIT au lieu de rester muette (000k)
      if(window.GBZone){ zone = GBZone.creer(); }
      else absente('cam_zone','lib/zone.js n\'est pas chargé sur cette page');
      lireCamera();
      // l'ouverture vient de donner les libellés : c'est MAINTENANT qu'on peut
      // vérifier qu'on est bien sur l'objectif voulu, et basculer sinon
      try{ await respecterObjectif(); }catch(e){}
      setTimeout(function(){
    ...
    function lireCamera(){
      ...
      prec = gris;
      if(zone) poser('cam_zone', zone.mesurer(gris).val);
    }
    /* ── changer d'objectif : LIBÉRER D'ABORD, demander ensuite ──
       PIÈGE PAYÉ SUR LE TÉLÉPHONE : la version précédente ouvrait le nouvel
       objectif AVANT de fermer l'ancien, pour ne jamais laisser l'écran noir.
       C'est exactement ce qu'Android refuse — les objectifs arrière partagent
       le même capteur physique et le second `getUserMedia` échoue en
       `NotReadableError`. Résultat vu par l'utilisateur : « j'appuie sur le
       bouton et rien ne se passe ». On coupe donc d'abord, puis on demande ;
       et si le nouvel objectif refuse, on rouvre l'ancien plutôt que de
       laisser l'appareil sans image. */
    async function changerCamera(contraintes){
      var ancienId = null;
      try{ ancienId = vstream.getVideoTracks()[0].getSettings().deviceId || null; }catch(e){}
      if(vstream){ try{ vstream.getTracks().forEach(function(t){ t.stop(); }); }catch(e){} }
      vstream = null;
      var neuf = null;
      try{ neuf = await navigator.mediaDevices.getUserMedia(contraintes); }
      catch(e){
        // rétablir l'ancien : un échec de changement ne doit jamais coûter l'image
        try{
          neuf = await navigator.mediaDevices.getUserMedia(ancienId
            ? {video:{deviceId:{exact:ancienId}}, audio:false}
            : {video:{facingMode:{ideal:'environment'}}, audio:false});
        }catch(e2){ neuf = null; }
        if(neuf){ vstream = neuf; if(vid){ vid.srcObject = neuf; try{ await vid.play(); }catch(e3){} } prec = null; if(zone) zone.raz(); }
        return false;
      }
      vstream = neuf;
      if(vid){ vid.srcObject = neuf; try{ await vid.play(); }catch(e){} }
      prec = null;                      // l'image de référence n'a plus de sens
      if(zone) zone.raz();              // le fond de blocs non plus : autre objectif, autre scène
      return true;
    }

    /* la liste des objectifs réellement présents, avec des noms qu'on peut
       DÉSIGNER. Les libellés bruts d'Android (« camera2 2, facing back ») ne
       veulent rien dire pour personne : on les traduit en « Arrière 1/2/3 »,
       ce qui donne enfin un vocabulaire pour dire « je veux l'arrière 3 ».
       Ce rang est aussi ce qu'on mémorise en second recours : sur certains
       téléphones l'identifiant d'un objectif CHANGE d'une session à l'autre,
       et un choix mémorisé par identifiant seul finissait par se perdre.
       Les libellés ne sont donnés par le navigateur QU'APRÈS une première
       autorisation — avant, ils sont vides, et il faut le dire. */
    async function objectifs(){
      // le nommage vit dans lib/objectif.js : UNE seule règle pour toute
      // l'application. Ce qui suit n'est qu'un repli si le module n'est pas
      // chargé sur la page.
      if(window.GBObjectif && GBObjectif.lister) return GBObjectif.lister();
      try{
    ...
    /* ── l'objectif préféré est RESPECTÉ, pas seulement mémorisé ──
       `facingMode:'environment'` ne dit que « une caméra arrière » : sur un
       téléphone qui en a trois, le navigateur choisit la sienne, souvent
       l'ultra grand-angle — image molle et déformée. On ouvre donc d'abord
       ce qu'on peut (c'est cette ouverture qui donne les libellés), puis on
       BASCULE sur l'objectif voulu s'il n'est pas déjà en place.
       Deux mémoires, dans cet ordre : l'identifiant exact, puis le rang parmi
       les arrières — parce que l'identifiant, lui, peut changer. */
    async function respecterObjectif(){
      var id = null, rang = null;
      try{ id = localStorage.getItem('gb-objectif'); }catch(e){}
      try{ rang = parseInt(localStorage.getItem('gb-objectif-rang'), 10) || null; }catch(e){}
      if(!id && !rang) return false;
      var actuel = null;
      try{ actuel = vstream.getVideoTracks()[0].getSettings().deviceId; }catch(e){}
      if(id && actuel && id === actuel) return false;      // déjà le bon
      var L = await objectifs();
      if(!L.length) return false;
      var cible = null;
      if(id) cible = L.find(function(o){ return o.id === id; }) || null;
      if(!cible && rang) cible = L.find(function(o){ return o.rang === rang; }) || null;
      if(!cible || cible.id === actuel) return false;
      var ok = await changerCamera({video:{deviceId:{exact:cible.id}}, audio:false});
      // l'identifiant a pu changer depuis la dernière fois : on remet à jour
      if(ok){ try{ localStorage.setItem('gb-objectif', cible.id); }catch(e){} }
      return ok;
    }
    ...
    return {
      ...
      objectifs: objectifs,
      respecterObjectif: respecterObjectif,
