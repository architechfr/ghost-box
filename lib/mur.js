...
window.GBMur = (function(){
  ...
  /* ── pourquoi cette plage-là, et pourquoi elle est LARGE ──
     L'ancienne plage (0,70 à 1,35) était trop resserrée pour que l'alternance
     se voie, et elle laissait un écart de 16 % entre colonnes sur 4000 gels.
     Élargie, le même harnais donne 3,9 % : plus les allures diffèrent, mieux
     la rotation égalise. Ce qu'il faut savoir en la relisant : élargir ne rend
     PAS le mur lisible — à 0,30, un mot passe encore en 303 ms sur une seule
     colonne à la fois, et une colonne lisible est une colonne VISABLE (voir
     `lecture` plus bas). L'équité et la lisibilité sont deux problèmes
     distincts ; celui-ci ne traite que l'équité. */
  var ALLURES = [0.30, 0.45, 0.70, 1.00, 1.35];
  var GEL_MS = 7000;
  /* ── LECTURE HORS MESURE ──
     Mesuré : à 330 px/s pour une case de 30 px, un mot passe en 67 à 130 ms,
     alors que lire un mot en demande 200 à 250. Le mur est donc illisible, et
     c'est assumé — mais « montrer, plutôt que garder pour soi » (8bis-c) perd
     son sens si personne ne peut voir ce qui est proposé.
     On ne peut pas ralentir en séance : la durée qui permet de LIRE un mot est
     exactement celle qui permet de le VISER. Mesuré sur 200 000 tentatives —
     v46 : mot reconnu au passage 2 % du temps, visé 0,00 % ; à vitesse
     lisible : reconnu 52 %, visé 21 %. Un mur lisible est un mur dirigeable.
     D'où ce mode, calqué sur la main de la planche (000ad) : on ralentit pour
     lire, et pendant ce temps RIEN NE PEUT ÊTRE RETENU. Le geste a le droit
     d'exister à condition de ne jamais pouvoir écrire. Il se coupe tout seul
     au bout de LECTURE_MAX — une lecture oubliée serait une séance perdue. */
  var LECTURE_PX  = 110;    // ≈ 273 ms par mot : lisible sans être confortable
  var LECTURE_MAX = 120000; // deux minutes, puis retour automatique à la mesure
  ...
  function creer(opts){
    ...
    /* lecture hors mesure : tLecture = instant d'entrée, ignorees = ce que la
       mesure a perdu pendant ce temps. On COMPTE ce qu'on refuse : un mot
       ignoré en silence serait un mensonge par omission. */
    var enLecture = false, tLecture = 0, ignorees = 0;
    ...
    /* Le bandeau de lecture. Il dit TROIS choses, et aucune n'est décorative :
       que rien n'est retenu (sinon on croit mesurer), combien de temps il
       reste (une contrainte temporaire se montre comme un progrès, 000af), et
       ce que la mesure a déjà perdu pendant ce temps. */
    function bandeauLecture(now){
      var reste = Math.max(0, LECTURE_MAX - (now - tLecture));
      g.fillStyle = 'rgba(3,7,10,.93)'; g.fillRect(0,0,W,44);
      g.fillStyle = 'rgba(226,104,95,.55)'; g.fillRect(0,43,W,1);
      g.textAlign='left'; g.textBaseline='alphabetic';
      g.font = '600 14px ' + TITRE; g.fillStyle = '#E2685F';
      g.fillText('LECTURE — RIEN N\'EST RETENU', 12, 20);
      g.font = '10px ' + POLICE; g.fillStyle = 'rgba(207,198,180,.8)';
      g.fillText('le mur ralentit pour être lu · un mur lisible est un mur visable'
        + (ignorees ? ' · ' + ignorees + ' mesure' + (ignorees>1?'s':'') + ' ignorée'
            + (ignorees>1?'s':'') : ''), 12, 35);
      g.textAlign='right'; g.fillStyle = 'rgba(120,134,142,.9)';
      g.font = '11px ' + POLICE;
      g.fillText(Math.ceil(reste/1000) + ' s', W-12, 35);
      g.fillStyle='rgba(226,104,95,.7)';
      g.fillRect(0, 41, W*(1 - reste/LECTURE_MAX), 2);
      g.textAlign='center'; g.textBaseline='middle';
    }
    ...
    function dessiner(now){
      if(mode === 'instant'){
        ...
        if(fige) voile(now);
        if(enLecture) bandeauLecture(now);
        return;
      }
      // ── escalier ──
      ...
      if(!fige){
        ...
        if(mode === 'instant' || esc.etape === 0){
          for(var i=0;i<cols.length;i++){
            ...
            /* en lecture, TOUTES les colonnes vont à la même allure lisible :
               garder la rotation laisserait trois colonnes sur cinq illisibles,
               et l'utilisateur croirait que le mode ne marche qu'à moitié */
            var v = enLecture ? LECTURE_PX : vitesse*c.allure;
            c.off = (c.off + v*dt/1000) % cycle;
          }
        }
        // la lecture se coupe seule : une lecture oubliée serait une séance perdue
        if(enLecture && (t - tLecture) > LECTURE_MAX) lecture(false);
    ...
    /* entrer et sortir de la lecture. À la sortie, on REND COMPTE de ce qui a
       été perdu : c'est le prix du mode, et il ne doit pas être découvert plus
       tard dans un journal. */
    function lecture(on){
      var veut = (on === undefined) ? !enLecture : !!on;
      if(veut === enLecture) return enLecture;
      enLecture = veut;
      if(enLecture){ tLecture = performance.now(); ignorees = 0; fige = false; }
      if(opts.onLecture) try{ opts.onLecture(enLecture, ignorees); }catch(e){}
      return enLecture;
    }
    ...
    function figer(info, test, tAttaque){
      ...
      /* LE VERROU. Pendant la lecture, le mur est lisible donc visable : ce
         qui en sortirait ne prouverait rien. On refuse, on COMPTE, et la page
         est prévenue par onIgnore — le refus doit s'entendre, pas se taire. */
      if(enLecture && !test){
        ignorees++;
        if(opts.onIgnore) try{ opts.onIgnore(info||'', ignorees); }catch(e){}
        return null;
      }
    ...
    return {
      ...
      /* lecture hors mesure : lecture() bascule, lecture(true/false) impose,
         enLecture() interroge sans rien changer */
      lecture: lecture,
      enLecture: function(){ return enLecture; },
      ignorees: function(){ return ignorees; },
      lecture_px: LECTURE_PX, lecture_max_ms: LECTURE_MAX,
