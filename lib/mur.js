...
window.GBMur = (function(){
  ...
  function css(){
    ...
    st.textContent =
      ...
      'flex-direction:row;align-items:stretch;gap:0;padding:0;margin:0}' +
      '.gbmur .gbm-col{flex:1 1 0;display:block;position:relative;overflow:hidden;' +
      'padding:0;margin:0;min-width:0;border-right:1px solid #212B31}' +
      '.gbmur .gbm-col:last-child{border-right:0}' +
      '.gbmur .gbm-hd{position:absolute;top:0;left:0;right:0;z-index:3;text-align:center;' +
      ...
      '.gbmur .gbm-strip{display:block;position:absolute;left:0;right:0;top:0;' +
      'padding:0;margin:0;overflow:visible;gap:0;will-change:transform}' +
      '.gbmur .gbm-cell{display:block;width:auto;flex:none;padding:0;margin:0;' +
      'height:' + ROW + 'px;line-height:' + ROW + 'px;text-align:center;' +
      'font-family:"IBM Plex Sans",system-ui,sans-serif;font-size:14px;color:#8FA0A9;' +
      'white-space:nowrap;overflow:hidden}' +
      '.gbmur .gbm-cell.hit{color:#0A0F12;background:#E9B44C;font-weight:600}' +
      '.gbmur .gbm-ligne{position:absolute;left:0;right:0;top:50%;height:' + ROW + 'px;' +
      ...
      '.gbmur .gbm-curseur{position:absolute;top:50%;height:' + ROW + 'px;' +
      'transform:translateY(-' + (ROW/2) + 'px);z-index:2;pointer-events:none;' +
      'border:2px solid #5FD3A6;border-radius:2px;box-shadow:0 0 12px rgba(95,211,166,.35);' +
      'transition:left .06s linear}' +
      '.gbmur .gbm-gel{position:absolute;inset:0;z-index:4;display:none;align-items:center;' +
      'justify-content:center;background:rgba(5,9,11,.72);text-align:center;padding:16px}' +
      '.gbmur .gbm-gel.on{display:flex}' +
      '.gbmur .gbm-mot{font-family:"Barlow Condensed","Arial Narrow",sans-serif;' +
      'text-transform:uppercase;letter-spacing:.06em;font-size:clamp(30px,11vw,80px);' +
      'line-height:1;color:#E9B44C;font-weight:600}' +
      '.gbmur .gbm-sub{font-family:ui-monospace,monospace;font-size:11.5px;color:#CFC6B4;' +
      'margin-top:10px;line-height:1.5}' +
      '.gbmur .gbm-gel.test .gbm-mot,.gbmur .gbm-gel.test .gbm-sub{color:#E2574C}';
  ...
  function creer(opts){
    ...
    listes.forEach(function(l, ci){
      var liste = melange(l);
      var d = document.createElement('div'); d.className='gbm-col';
      var h = document.createElement('div'); h.className='gbm-hd'; h.textContent = FAMILLES[ci].t+' · '+liste.length;
      var st = document.createElement('div'); st.className='gbm-strip';
      // la liste est écrite deux fois : le défilement boucle sans coupure visible
      // un mot coupé en plein milieu ne serait plus le mot proposé : les longs
      // sont écrits plus petit pour tenir en entier dans la colonne
      for(var k=0;k<2;k++) liste.forEach(function(w){
        var c=document.createElement('div'); c.className='gbm-cell'; c.textContent=w.mot;
        if(w.mot.length>9) c.style.fontSize = Math.max(9, Math.round(14*9/w.mot.length)) + 'px';
        st.appendChild(c);
      });
    ...
    var ligne = document.createElement('div'); ligne.className='gbm-ligne';
    var curseur = document.createElement('div'); curseur.className='gbm-curseur';
    var gel = document.createElement('div'); gel.className='gbm-gel';
    var gelMot = document.createElement('div'); gelMot.className='gbm-mot'; gelMot.textContent='—';
    var gelSub = document.createElement('div'); gelSub.className='gbm-sub';
    ...
    function degeler(){
      fige = false; gel.className = 'gbm-gel';
      var h = hote.querySelectorAll('.gbm-cell.hit');
    ...
    function figer(info, test){
      ...
      gelSub.innerHTML = test
        ...
        : String(info || '');
      gel.className = 'gbm-gel on' + (test ? ' test' : '');
      if(!test) c.retenus++;
