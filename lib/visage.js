...
/* -- seuils PRUDENTS, assouplissables explicitement --
   Meme principe que pour la silhouette : on peut baisser la barre pour un
   lieu ou les visages sont loin, mais alors on en invente davantage. Le choix
   appartient a l'utilisateur, et il doit etre dit. */
let cCONF = CONF_MIN, cSTREAK = MIN_STREAK, cTAILLE = MIN_TAILLE;
export function regler(o){
  o = o || {};
  cCONF   = o.confiance != null ? o.confiance : CONF_MIN;
  cSTREAK = o.suite     != null ? o.suite     : MIN_STREAK;
  cTAILLE = o.taille    != null ? o.taille    : MIN_TAILLE;
  return reglages();
}
export function reglages(){ return { confiance:cCONF, suite:cSTREAK, taille:cTAILLE }; }
...
export async function load(){
  ...
  chargement = (async () => {
    ...
    detecteur = await vision.FaceDetector.createFromOptions(fileset, {
      ...
      runningMode: 'VIDEO',
      minDetectionConfidence: 0.5   /* le tri severe est fait par juger(), pas par le modele : c'est ainsi qu'on peut EXPLIQUER un refus */
    });
    return detecteur;
  })();
...
function juger(d, H){
  ...
  if(conf < cCONF){ lastWhy = 'confiance trop faible'; return false; }
  if(bb.height / H < cTAILLE){ lastWhy = 'trop petit pour être sûr'; return false; }
...
export function detect(video, ts){
  ...
  if(streak < cSTREAK){
    lastWhy = 'en cours de confirmation (' + streak + '/' + cSTREAK + ')';
