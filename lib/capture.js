...
window.GBCapture = (function(){
  ...
  function css(){
    ...
    st.textContent =
      ...
      'background:rgba(10,15,18,.8);padding:1px 6px;border-radius:9px}' +
  ...
  function takePhoto(){
    ...
    c.toBlob(function(blob){
      ...
      save({ id: Date.now(), nom: nom, mime: 'image/jpeg', taille: blob.size,
             duree: 0, vignette: vignette, cam: cfg.label || 'capture',
             blob: blob, type: 'photo' });
      count++;
      if(counter){ counter.textContent = count + ' ✓'; counter.style.display = 'block'; }
    }, 'image/jpeg', 0.92);
  }
  ...
  return { init: init, photo: takePhoto };
})();
