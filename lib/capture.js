...
window.GBCapture = (function(){
  ...
  function photoCanvas(c, extra){
    if(!c || !c.width) return;
    // utilisable SANS init() : la séance simple a son propre déclencheur et
    // n'affiche pas les boutons flottants de ce module. Sans ce repli, `cfg`
    // restait null et la photo échouait silencieusement — mesuré au test.
    if(!cfg) cfg = {};
    blink();
